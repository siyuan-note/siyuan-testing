...
The normal test commands automatically create and use `SiYuan-Testing` under the current user's home directory. Each command compiles and watches the desktop bundle from the sibling `../siyuan/app` checkout, always starts `app/kernel/SiYuan-Kernel`, runs Playwright, and stops both the kernel and compiler in cleanup even when the tests fail. Using the developer-built kernel keeps frontend configuration such as `entryVisibility` aligned with the current source. The managed commands refuse to reuse an already occupied target so they never stop a process they did not start. Set `SIYUAN_APP_DIR` when the app path is elsewhere.

The default target is `http://127.0.0.1:6807`, which allows the test workspace to run beside a normal SiYuan instance on port 6806. Set `SIYUAN_BASE_URL` to use another local address. Every local test run uses the same home-directory test workspace; ports, branches, and debugging sessions must not create workspace variants. Non-loopback targets are rejected unless `SIYUAN_ALLOW_REMOTE=1` is explicitly set, and `SIYUAN_EXPECT_WORKSPACE` is only used to validate the absolute workspace path of an intentionally remote target. The local test workspace is created automatically before target validation.
...
Navigation cases tap exact text offsets in two paragraphs, then tap the top-bar back and forward buttons and assert the restored block ID and both selection offsets. A second case taps a different offset in the first paragraph before leaving, checking that back restores its latest position. Tap coordinates come from DOM text ranges; expected offsets do not come from the application's navigation history. Run these cases alone with `pnpm test:ipad --project=ipad tests/ipad/editor-navigation-history.spec.ts`.
...
List mind map regressions run with `pnpm test:focused tests/editor-list-mindmap.spec.ts`. They cover side selection in both directions from both margins, extending and shrinking across adjacent blocks, cleanup after mouse release, clicking the canvas or a node to clear block selection, editing an empty node without placeholder text, and the 32-by-32 unnamed root. Editing cases verify saved content through the HTTP API and after reload; list hierarchy checks compare the live DOM with the persisted `.sy` tree.
